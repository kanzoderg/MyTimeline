# File: kemonodl.py
# Kemono.party / Coomer.party downloader
...
kemono_domain_thumb = "img.kemono.cr/thumbnail/data/"
coomer_domain = "coomer.st"
coomer_domain_thumb = "img.coomer.st/thumbnail/data/"

allowed_kemono_types = ["patreon", "fanbox", "gumroad"]
allowed_coomer_types = ["onlyfans"]
...
parser.add_argument(
    "-thumb",
    "--thumbnails-only",
    help="Download only thumbnail images (if the original file does not currently exist). If you choose to download original files in the future, the existing thumbnail will be deleted after successfully downloading the original file.",
    action="store_true",
)
...
def check_exists(filepath):
    if os.path.exists(filepath):
        ...
        if filepath.split(".")[-1] in ["zip", "rar", "7z"]:
            ...
            if os.path.exists(unzip_path):
                print(
                    f"--- Notice: File {filepath} is possibly unzipped as {unzip_path}, skipping."
                )
                print(
                    f"--- If you want to re-download, please remove the existing file or directory."
                )
                return True
    return False
...
def get_user_data(uid, type_):
    if type_ in allowed_kemono_types:
        profile_api = f"https://{kemono_domain}/api/v1/{type_}/user/{uid}/profile"
    elif type_ in allowed_coomer_types:
...
def main(url):
    ...
    if type_ not in allowed_kemono_types + allowed_coomer_types:
        print(f"Unsupported type: {type_}")
        return
    name, post_count, profile_data = get_user_data(uid, type_)
    user_dir = os.path.join(args.output, name)
    ...
    for offset in range(0, post_count, 50):
        if "post/" in url:
            posts_data = [
                {
                    "id": re.search(r"/post/([a-zA-Z0-9]+)", url).group(1),
                }
            ]
            post_count = 1
        else:
        ...
        for post in posts_data:
            ...
            for file in files:
                ...
                internal_filename = file["path"].split("?")[0].strip("/").split("/")[-1]
                thumbnail_filename = f"{pid}_thumbnail_{internal_filename}"
                if not check_allowed_filename(filename):
                    print(f"File with invalid filename: {filename}")
                    filename = f"{pid}_{internal_filename}"
                    print(f"Renamed to: {filename}")
                if type_ in allowed_kemono_types:
                    url = f"https://{kemono_domain}{file['path']}"
                    thumb_url = f"https://{kemono_domain_thumb}{file['path']}"
                elif type_ in allowed_coomer_types:
                    url = f"https://{coomer_domain}{file['path']}"
                    thumb_url = f"https://{coomer_domain_thumb}{file['path']}"
                else:
                    raise Exception(f"Unsupported type: {type_}")
                
                filepath = os.path.join(user_dir, filename)
                thumbpath = os.path.join(user_dir, thumbnail_filename)

                target_filename = thumbnail_filename if args.thumbnails_only else filename
                target_path = thumbpath if args.thumbnails_only else filepath
                target_url = thumb_url if args.thumbnails_only else url

                if not check_exists(filepath):
                    if args.thumbnails_only:
                        if os.path.exists(thumbpath):
                            print(
                                f"Thumbnail for {filename} already exists, skipping thumbnail download."
                            )
                            continue
                        if thumbpath.split(".")[-1] not in ["jpg", "jpeg", "png", "gif", "bmp", "webp"]:
                            print(
                                f"Thumbnail URL {thumb_url} does not seem to be an image, skipping thumbnail download."
                            )
                            continue
                    ...
                    try:
                        ...
                        if result:
                            print(f"Not downloaded {target_filename}")
                        else:
                            print(f"Saved {target_filename}")
                            # remove thumbnail if original file is successfully downloaded
                            if (not args.thumbnails_only) and os.path.exists(
                                thumbpath
                            ):
                                os.remove(thumbpath)
                                print(
                                    f"Removed existing thumbnail {thumbnail_filename} since original file is downloaded."
                                )
                    except Exception as e:
